...
const canonicalDepdevOrganization = 'Department of Economy, Planning, and Development (DEPDev)';
const isDepdevOrganization = organization =>
  /DEPDev|Department of Economy, Planning, and Development/i.test(organization);
assert(speakerRecords.filter(speaker => isDepdevOrganization(speaker.org))
  .every(speaker => speaker.org === canonicalDepdevOrganization),
  'Every public DEPDev affiliation must use the full department name without a unit suffix.');
assert(roster.roster.filter(record => isDepdevOrganization(record.organization))
  .every(record => record.organization === canonicalDepdevOrganization),
  'Every roster DEPDev affiliation must use the full department name without a unit suffix.');
...
assert(photoSpeakers.length === 36, `Expected 36 supplied resource-person photos; found ${photoSpeakers.length}.`);
...
assert(rosterPhotoRecords.length === 33,
  `Expected 33 roster records with supplied portraits; found ${rosterPhotoRecords.length}.`);
...
const expectedNewPortraits = [
  ...
  ['reinald-adrian-d-pugoy', 'Reinald Adrian D. Pugoy', 'assets/speakers/2026/reinald-adrian-d-pugoy.jpg'],
  ['david-joseph-emmanuel-b-yap-jr', 'David Joseph Emmanuel B. Yap Jr.', 'assets/speakers/2026/david-joseph-emmanuel-b-yap-jr.jpg'],
  ['yuko-lisette-r-domingo', 'Yuko Lisette R. Domingo', 'assets/speakers/2026/yuko-lisette-r-domingo.png']
];
for (const [id, displayName, photo] of expectedNewPortraits) {
  assert(roster.roster.some(record => record.id === id && record.photo === photo),
    `${id} must retain its verified roster portrait mapping.`);
...
assert((speakerManifest.match(/"manual-square-crop"/g) || []).length === 36,
