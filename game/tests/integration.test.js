...
test('homepage scripts parse and the mobile frame remains usable', () => {
  ...
  assert.match(releaseCss, /#program \.section-shell > \.program-item \{[\s\S]*?padding-block: 0 !important;/);
  assert.match(releaseCss, /#program \.section-shell \.program-row \{[\s\S]*?grid-template-columns: 52px minmax\(0, 1fr\) !important;/);
  assert.match(releaseCss, /\.program-row__time \{[\s\S]*?grid-row: 1 \/ 3 !important;/);
